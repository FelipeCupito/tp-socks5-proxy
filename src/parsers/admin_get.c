...
const uint8_t STATUS_ERROR_INVALID_ACTION = 0x01;
const uint8_t STATUS_ERROR_INVALID_OPTION = 0x02;
const uint8_t STATUS_ERROR = 0x03;

const uint8_t GET_ACTION = 0X00;
...
int admin_get_marshall(buffer *b, const uint8_t status, char *res, int res_size) {
  ...
  size_t size;
  uint8_t *buff = buffer_write_ptr(b, &size);
